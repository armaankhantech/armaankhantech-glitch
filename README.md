# Hey, I'm Armaan Khan 👋

**AI Automation Engineer (in progress)** · Building intelligent systems that save businesses time and money

📍 Pune, India | 🎯 Goal: First freelance client by Month 3 | 📅 Day 16 of public journey

---

## 🤖 What I Actually Build

I design and build **AI-powered automation workflows** using:

- **n8n** — workflow automation (triggers, webhooks, logic, integrations)
- **Ollama** — local LLMs for AI reasoning inside workflows
- **PostgreSQL** — structured data storage for automation outputs
- **Python** — scripting and glue code for automations
- **Google Sheets / Forms / Gmail** — business-facing integrations
- **Docker** — local infrastructure for running everything
- **REST APIs & JSON** — connecting everything together

---

## 🚀 Current Portfolio Projects

| #  | Project                          | Status           | Stack                                        |
|----|----------------------------------|------------------|----------------------------------------------|
| 01 | AI Lead Analyzer CRM             | ✅ Complete       | n8n, Ollama, PostgreSQL, Google Sheets, Gmail |
| 02 | ARK AI Customer Support Assistant| 🔨 In Progress    | n8n, Ollama, PostgreSQL, ngrok               |
| 03 | AI Content Automation System     | 📋 Planned        | n8n, Claude API                              |
| 04 | Lead Generation Automation       | 📋 Planned        | n8n, REST APIs                               |
| 05 | AI Business Operations Automation| 📋 Planned        | TBD                                          |

---

## 🏆 Certifications

| Certification                              | Issuer    | Year |
|--------------------------------------------|-----------|------|
| Claude 101                                 | Anthropic | 2026 |
| AI Fluency: Framework & Foundations (4D)   | Anthropic | 2026 |

---

## 📖 My Public Journey

I document everything I learn, build, and ship — daily.

- 🔗 **LinkedIn:** [linkedin.com/in/armaankhan-tech](https://www.linkedin.com/in/armaankhan-tech/)
- 🐦 **Twitter/X:** [@armaankhantech](https://x.com/armaankhantech)
- 📁 **Journey Repo:** [ai-automation-journey](https://github.com/armaankhantech/Ai-automation-journey)

---

## 🧭 Roadmap (5 Phases)

Phase 1: Foundations ✅ Complete (Days 1–5)

Phase 2: Builder 🔨 In Progress (Days 6–30)

Phase 3: Professional 📋 Planned

Phase 4: Expert 📋 Planned

Phase 5: Entrepreneur 📋 Planned

---

## 🛠️ Tools & Tech

![n8n](https://img.shields.io/badge/n8n-EA4B71?style=flat&logo=n8n&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=flat&logo=postgresql&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Ollama](https://img.shields.io/badge/Ollama-000000?style=flat&logoColor=white)
![Google Sheets](https://img.shields.io/badge/Google%20Sheets-34A853?style=flat&logo=google-sheets&logoColor=white)
![REST API](https://img.shields.io/badge/REST%20API-000000?style=flat&logo=fastapi&logoColor=white)

---

*Building in public. Every day. No shortcuts.*
